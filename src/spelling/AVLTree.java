...
public class AVLTree<E extends Comparable<E>> {
    ...
    public void save(String fileName) {
        if (root != null) {
            ...
            saveSub(root, out); // call the recursive method
            out.close();
        }
    }

    private void saveSub(Node node, PrintWriter out) {
        // you are required to complete this method
        //code will be similar to code in BinarySearchTree printSub() method – 2 recursive calls and output statement
        // replace SOP with:
        if (node != null) {
            saveSub(node.left, out);
            out.println(node.key);
            System.out.println("saved");
            saveSub(node.right, out);
        }

        //which traversal will you use – preorder, inorder or postorder?
    }
}
